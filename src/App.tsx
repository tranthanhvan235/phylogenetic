...
const App: React.FC = () => {
  ...
  const fetchWeatherByCoords = async (lat: number, lon: number) => {
    ...
    try {
      ...
      setWeather({
        ...response.data,
        icon: response.data.weather[0].icon // Lưu icon từ API
      })
  ...
  const fetchWeather = async () => {
    ...
    try {
      ...
      setWeather({
        ...response.data,
        icon: response.data.weather[0].icon // Lưu icon
      })
    } catch (error) {
      console.error('Lỗi khi lấy dữ liệu thời tiết:', error)
      setWeather(null)
  ...
  return (
    <div className='container'>
      <h1>Weather App</h1>
      <input type='text' placeholder='Nhập tên thành phố...' value={city} onChange={(e) => setCity(e.target.value)} />
